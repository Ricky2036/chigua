...
h = src
h = re.sub(r"<style[^>]*>.*?</style>", '<link rel="stylesheet" href="../assets/theme.css?v=20260903c">', h, count=1, flags=re.S)
# 只替换页面末尾那个主脚本（保留 body 开头的防闪烁内联脚本）
last = h.rfind("<script>")
h = h[:last] + '<script src="../assets/reader.js?v=20260903c"></script>\n</body>\n</html>\n'
...
def build_topic_page(tpl, cfg):
    ...
    if ovr:
        # 必须用正则：模板里 href 带版本号（?v=20260903c），
...
<head>
    ...
    <link rel="stylesheet" href="./assets/theme.css?v=20260903c">
    <style>
...
__HOME_BODY__
    ...
    <script src="./assets/reader.js?v=20260903c"></script>
</body>
</html>
'''
